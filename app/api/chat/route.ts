...
export async function POST(request: NextRequest) {
  try {
    ...
    const imageResults = contextItems.filter(item => item.type === 'image' && item.imageUrl);
    ...
    const systemPrompt = `You are an expert research assistant specializing in Himalayan climate, environmental variables, glaciology, and atmospheric science. You have access to a comprehensive knowledge base of peer-reviewed research papers.
...
Remember: Users will see the actual images, so describe what they show and why they're relevant.`;
    ...
    const messages: OpenAI.ChatCompletionMessageParam[] = [
      ...
      {
        role: 'user',
        content: `Research Context:\n${contextString}\n\nUser Question: ${message}\n\nRemember to cite papers by their FULL TITLE, not as "Source X". If relevant figures are available, mention them in your response.`,
      },
    ];
